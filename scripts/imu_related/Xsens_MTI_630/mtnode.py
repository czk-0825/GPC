...
from gaze_pros_control import mtdevice_util as mtdevice
from gaze_pros_control import mtdef_util as mtdef
...
class XSensDriver(object):
    ...
    def __init__(self):
        ...
        self.time_shift = rospy.Duration(0.041824) # 0.0415584
        self.cam_to_imu = np.array([
            [ 0.99999543, 0.00282365, -0.00108008, -0.00812752],
            [-0.00104855, -0.01114514, -0.99993734, -0.01689469],
            [-0.00283551, 0.9999339, -0.01114213, 0.01967506],
            [0.0, 0.0, 0.0, 1.0]
])
    ...
    def spin_once(self):
        ...
        # common header
        self.h = Header()
        self.h.stamp = rospy.Time.now() + self.time_shift
        ...
        if self.pub_imu:
            ...
            if self.imu_pub is None:
                ...
                self.imu_color_pub = rospy.Publisher('/imu_color_to_world_pub', PoseStamped, queue_size=10)
            T_world_base = np.eye(4)
            T_world_base[:3, 3] = [0, 0, 1.5]
            R_world_base = R.from_euler('xyz', [0, 0, 0]).as_matrix()
            T_world_base[:3, :3] = R_world_base

            qx, qy, qz, qw = self.imu_msg.orientation.x,self.imu_msg.orientation.y,self.imu_msg.orientation.z,self.imu_msg.orientation.w
            if np.sqrt(qx**2 + qy**2 + qz**2 + qw**2) != 0:
                ...
                T_camera_link_to_optical = np.array(self.cam_to_imu)
                ...
                self.imu_color_pub.publish(pose_msg)
