...
# ser_port = "/dev/ttyUSB4"

buffer_name = sys.argv[1]
# buffer_name = "right_thigh"
...
imu_buffer = np.memmap(current_dir+"/log/{}.npy".format(buffer_name), dtype='float32', mode='r+',shape=(13,))
use_Acc = True
...
def Cmd_RxUnpack(buf, DLen):
    ...
    if buf[0] == 0x11:
        ...
        if ((ctl & 0x0040) != 0):
            ...
            print(buffer_name+": %.2f, "%imu_dat[19]+"%.2f, "%imu_dat[20]+"%.2f"%imu_dat[21])
...
def read_data():
    cmd = cal_cmd(rate=int(100),data_type="0067")# data_type必须为4位
