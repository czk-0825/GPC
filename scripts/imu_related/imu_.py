...
elif buffer_name == "head":
    mac_address = "9b:f6:48:f9:ee:93"
    buffer_name = "imu_head"
...
class AnyDevice(gatt.Device):
    ...
    imu_buffer = np.memmap("/home/czk/catkin_ws/src/samcon_perception/scripts/imu/{}.npy".format(buffer_name), 
    ...
    def services_resolved(self):
        ...
        params[5] = 150  # 数据主动上报的传输帧率[取值0-250HZ], 0表示0.5HZ
        params[6] = 0  # 陀螺仪滤波系数[取值0-2],数值越大越平稳但实时性越差
        params[7] = 0  # 加速计滤波系数[取值0-4],数值越大越平稳但实时性越差
        params[8] = 0  # 磁力计滤波系数[取值0-9],数值越大越平稳但实时性越差
    ...
    def parse_imu(self, buf):
        ...
        if buf[0] == 0x11:
            ...
            if ((ctl & 0x0040) != 0):
                ...
                # print("\r%s:X:%.3f,Y:%.3f,Z:%.3f"%(buffer_name, float(tmpX), float(tmpY), float(tmpZ)),end='         ')
                # print(buffer_name)
